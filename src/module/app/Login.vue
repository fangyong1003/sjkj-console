<!-- Created by shi.pengyan on 2016-11-08. -->
<template >
  <div class="login-wrapper" :style="{backgroundImage: 'url(' + img + ')'}" style="background-size: 100% 100%" >
    <!--<div id="particles-js" style="position:absolute;left:0;top:0;right:0;bottom:0;"></div>-->

   <!-- <header id="header">
      <div>
        &lt;!&ndash;<span class="app-name">{{$t('PRODUCT_BRAND')}}</span>&ndash;&gt;
      </div>
    </header>-->
      ...
      <div id="content" class="container">
        <el-row>
          ...
          <el-col :span="7" style = "position:relative">
            <div class="grid-content bg-purple login-form-wrapper" style = "position:absolute;top:150px;left:20px">
...
<style lang="scss">
  ...
  .login-form-header {
    ...
    /*font-family:FZYaoti,Verdana,Sans-serif;*/
    font-weight: bold;
    font-size: large;
  }
  }
      ...
      loginHandler () {
        ...
        this.$refs.form.validate((valid) => {
          if (valid) {
            ...
            this.login(loginInfo).then((data) => {
              if (data.login) {
                console.log('login Success');

                this.$router.push('/dashboard');
              } else {
                MessageBox.alert('登陆失败请检查用户名或密码！', 'Error', {type: 'error'});
              }
