                ...
                <ul class="ruleList">
                  ...
                  <div v-for="(val, key) in ruleValueList">
                    ...
                    <li style="width: 50px;padding-bottom: 10px">
                      <el-button icon="delete" @click="delRuleValue(key)" v-show="val.disable!==false"/>
                    </li>
                  </div>
            ...
            <el-row>
              ...
              <ul class="ruleList">
                ...
                <div v-for="(val, key) in ruleTwoValueList">
                  ...
                  <li style="width: 50px;padding-bottom: 10px">
                    <el-button icon="delete" @click="delRuleTwoValue(key)" v-show="val.disable!==false"/>
                  </li>
                </div>
                <li style="padding-bottom: 30px">
                  &nbsp;
    ...
    methods: {
      ...
      saveHandler() {
        this.$refs.form.validate((valid) => {
          ...
          let self = this;
          this.detail.desc = this.description;
          this.detail.photos = this.detailPhotos;
          this.detail.itemId = this.productId;
          this.skus = [];
          this.specs = [];
          if (valid) {
                ...
                if (this.ruleTwoValueList.length > 0) {
                  for (let j = 0; j < this.ruleTwoValueList.length; j++) {
                    if (this.ruleTwoValueList[j].ruleValue !== '') {
                      ...
                      this.specsChild.value = this.specsValue;
                      this.skuSpecs.push(this.specsChild);
                      this.skusChild = {'specs': '', 'id': '', 'itemId': ''};
                      this.skusChild.id = this.ruleDetailList[i * this.ruleTwoValueList.length + j].id;
                      this.skusChild.itemId = this.productId;
                  ...
                  this.skuSpecs.push(this.specsChild);
                  this.skusChild = {'specs': '', 'id': '', 'itemId': ''};
//                  alert(this.ruleDetailList[i].id);
                  this.skusChild.id = this.ruleDetailList[i].id;
                  this.skusChild.itemId = this.productId;
                  this.skusChild.specs = this.skuSpecs;
                  this.skus.push(this.skusChild);
                }
...
//                this.imageUrl = product.detail.photos[0];
              ...
              if (product.specs.length > 1) {
                ...
                for (let i = 0; i < product.specs[0].values.length; i++) {
                  this.ruleValueList.push({'ruleValue': product.specs[0].values[i].valueName, 'disable': false});
                }
                for (let i = 0; i < product.specs[1].values.length; i++) {
                  this.ruleTwoValueList.push({'ruleValue': product.specs[1].values[i].valueName, 'disable': false});
                }
...
//                this.loadListFlag = false;
      ...
      getRuleList() {
        if (this.loadListFlag) {
          let oldRuleDetailList = this.ruleDetailList;
          let ifIn = true;
          this.ruleDetailList = [];
          for (let i = 0; i < this.ruleValueList.length; i++) {
            if (this.ruleValueList[i].ruleValue !== '') {
              if (this.ruleTwoValueList.length > 0) {
                for (let j = 0; j < this.ruleTwoValueList.length; j++) {
                  if (this.ruleTwoValueList[j].ruleValue !== '') {
                    ifIn = true;
                    for (let q = 0; q < oldRuleDetailList.length; q++) {
                      if (oldRuleDetailList[q].ruleOne === this.ruleValueList[i].ruleValue && oldRuleDetailList[q].ruleOne === this.ruleTwoValueList[j].ruleValue) {
                        this.ruleDetailList.push(oldRuleDetailList[q]);
                        ifIn = false;
                      }
                    }
                    if (ifIn) {
                ...
                ifIn = true;
                for (let q = 0; q < oldRuleDetailList.length; q++) {
                  if (oldRuleDetailList[q].ruleOne === this.ruleValueList[i].ruleValue) {
                    this.ruleDetailList.push(oldRuleDetailList[q]);
                    ifIn = false;
                  }
                }
