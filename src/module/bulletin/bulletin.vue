    ...
    <el-table ref="bulletinTable" :data="getSysBulletins" height="500" border :stripe="true"
      ...
      <span>
        ...
        <el-button :type="row.state === 1?'danger':'info'" size="small"
                   @click="reviewHandler($index,row)">{{row.state === 1?'审核':'取消审核'}}
        </el-button>
      </span>
      </el-table-column>
    </el-table>
...
<script type="text/babel">
  ...
  export default{
    ...
    methods: {
      ...mapActions(['queryBulletin', 'querySysBulletins', 'deleteSysBulletin', 'querySysBulletinsCount', 'queryGroup', 'reviewSysBulletin', 'announcement']),
      initPage() {
      ...
      reviewHandler(index, row) {
        this.$confirm(row.state === 1 ? '你确定要审核这条公告吗?' : '你确定要取消这条公告的审核吗?', '确定', {
          ...
          this.reviewSysBulletin(row).then(data => {
            ...
            if (row.state !== 1) {
              this.announcement({title: row.title, content: row.content});
            }
          });
        });
      },
